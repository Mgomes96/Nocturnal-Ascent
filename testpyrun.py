...
#tke= vars['tke'][:] #subgrid tke for LES testcase
 ...
 #   fig=plt.figure(figsize=(10,10))
  ...
  #  ax=fig.add_subplot(2,1,1)
   ...
   # ax.set_xlim([-2968,2968])
    ...
    #plt.pcolormesh(xm,zm,w[k,:-1,0,:],cmap=bwr_custom,vmin=-0.1, vmax=0.1)
...
for k in range(0,len(time),1):
    ...
    plt.pcolormesh(xm,zm,cloud[k,:,0,:],cmap='seismic')
...
#for k in range(0,len(time),1):
  ...
  #  ax=fig.add_subplot(2,1,1)
  #  plt.pcolormesh(xm,zm,theta[k,:,0,:],cmap='CMRmap_r',vmin=300.0, vmax=379.0)
  #  plt.colorbar(label='Potential temperature (K)')
# #  plt.title(time2[k] + '     ' +str(int(solrad[k,0,0])),name='Arial',weight='bold',size=20)
  #  plt.xlabel('X Domain (km)',name='Arial',weight='bold',size=16,style='italic')
  #  plt.ylabel('Height (km)',name='Arial',weight='bold',size=16,style='italic')
 ...
 #   ax=fig.add_subplot(2,1,2)
 #   plt.pcolormesh(xm,zm,qv[k,:,0,:],cmap='CMRmap', vmin=0, vmax=0.0075)
...
#Animation of zoomed in U wind and theta
#xm,zm=np.meshgrid(xh,z)
...
#Animation of zoomed in U wind and tke
xm,zm=np.meshgrid(xh,z)
...
for k in range(0,len(time),1):
    ...
    plt.xlabel('Subgrid TKE (m^2/s)',name='Arial',weight='bold',size=16,style='italic')
    plt.ylabel('Height (km)',name='Arial',weight='bold',size=16,style='italic')
    #ax.set_xlim([0.0,150])
    ax.set_xlim([0.0,150])
    ax.set_ylim([0,3])
